...
*/
    public int minDistance(String word1, String word2) {
        int[][] opt = new int[word1.length()][word2.length()];
        for (int[] row: opt) {
            Arrays.fill(row, -1);
        }

        return levenshteinDistance(word1, word1.length() - 1, word2, word2.length() - 1, opt);
    }

    private int levenshteinDistance(String s1, int s1Index , String s2, int s2Index,
                                           int[][] opt) {
        if (s1Index < 0) {
